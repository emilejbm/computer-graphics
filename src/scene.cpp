...
Scene::Scene() {
    ...
    tms[1].SetCenter(V3(50.0f, 0.0f, -250.0f));
    //tms[1].SetTexturedCube(tms[1].GetCenter(), 50.0f);
...
//    }
...

    {
        ...
        clock_t ticks, delta_ticks;

        clock_t delta_time = 0;
        unsigned int frames = 0;
        double fps = 30;
        double averageFrameTimeMS = 33.333;

        hwfb->fps = 0;
        ostringstream ss;
        string title_str = "first person HW. FPS: ";
        string fps_str = "0";
        ...
        for (int fi = 0; fi < fN; fi++) {
            ...
            delta_ticks = clock() - ticks;
            if(delta_ticks > 0){
                hwfb->fps = (CLOCKS_PER_SEC / delta_ticks) / 100;
                ss << hwfb->fps;
                fps_str = title_str + ss.str();
                cout << fps_str << endl;
            }

            hwfb->label(fps_str.c_str());

            hwfb->redraw();
            Fl::check();
            ppc->C = ppc->C + V3(0.0f, 0.1f, 0.0f);
...

    {
        int fN = 10000;
        for (int fi = 0; fi < fN; fi++) {
            hwClearColor = V3(0.0f, 0.0f, (float)fi / (float)(fN - 1));
            hwfb->redraw();
            Fl::check();
        }
        return;
...
void Scene::RenderHW() {
    ...
    if (needInitHW) {
        ...
        unsigned char *data = stbi_load("/home/emilejbm/Desktop/github/CS334-2022Fall/CS334/hw1_v2/tiffs/carpet_small.jpg", &width, &height, &channelsN, 4);
        if (data){
    ...
    //ppc->SaveToTextFile();
//    for (int tmi = 0; tmi < tmsN; tmi++) {
//        tms[tmi].RenderTexturedHW(&texture);
//    }

    tms[1].RenderHW();
    tms[0].RenderTexturedHW(&texture);
